	...
	*/
	...
	class RevenueManager extends Library
	{
		...
		public function getAllRevenueChannels()
		{
            ...
            if($role != SystemAdmin) {
                $results = $this->selectQuery('revenue_channel','*',"created_by = '".$_SESSION['mf_id']."'");
            }else{
		...
		public function addRevenueChannel($post){
			extract($_POST);
			if(!checkForExistingEntry('revenue_channel', 'revenue_channel_name', $revenue_channel_name)){
				if(!checkForExistingEntry('revenue_channel', 'revenue_channel_code', $revenue_channel_code)){
					$add_revenue_channels="INSERT INTO revenue_channel(revenue_channel_name,revenue_channel_code,created_by)
    			                       VALUES('".$revenue_channel_name."', '".$revenue_channel_code."', '".$_SESSION['mf_id']."')";
		...
		public function getAllServiceOptions(){
            ...
            if($role != SystemAdmin){
                $service_channels = $this->selectQuery('all_service_options','*',"created_by = '".$_SESSION['mf_id']."'");
            }else{
                $service_channels = $this->selectQuery('all_service_options','*');
            }
        ...
        public function getAllServiceBills(){
            ...
            if($role != SystemAdmin){
                $service_bills = $this->selectQuery('all_service_bills','*',"created_by = '".$_SESSION['mf_id']."'");
            }else{
                $service_bills = $this->selectQuery('all_service_bills','*');
            }
